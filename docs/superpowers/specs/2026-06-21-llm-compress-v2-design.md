...
### ⑤ Log(改写 log.rs)— 级别评分保留

> **实现注**:模板挖掘(RLE)在最终评审中确认永远无净收益(变量内联表格开销 ≥ 折叠节省),已从 log.rs 移除。Log 压缩器现仅做级别评分保留(删行)。

- **级别评分保留(删行)**:`score::line_score`,`cfg.log.keep_levels`(默认 `["error","warn"]`)对应级别 + 栈帧必留,DEBUG/INFO 超量按分丢弃;保留首尾 + 高分中段错误。参考 log_compressor.rs。
- 发生删行 → `lossy=true` 挂 CCR(`kind=Text`)。
