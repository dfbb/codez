...
| `FunctionCallOutput` | 翻译(`role:"tool"`) | 翻译(`tool_result`) | 标准函数工具结果 |
...
2. **`auth_key` 内联** → **仅允许出现在 gitignored 的 `zmod/llm-switch/tests/testkey.toml`**,供离线/实跑测试与独立运行;正式 `~/.codex/config-zmod.toml` 出现 `auth_key` 应告警(或拒绝)。
...
3. 上述硬不变量满足。
4. core 触点仅 §6 所列(Cargo 依赖 + `ModelClient::new` 形参 + `stream_responses_api` 一处赋值改写),不改既有错误/遥测/流映射逻辑。
