...
> **命名空间工具的源头降级**：codex 默认 `namespace_tools = true`，会把多智能体/协作等工具打包成 `{"type":"namespace"}` 发出，撞上面的硬失败。为此 `002-llm-switch.patch` 在 `core/src/tools/spec_plan.rs` 的 `namespace_tools_enabled()` 加了判断——**被本 crate `route()` 接管的 provider 一律返回 `false`**，复用 codex 原生的 namespace 降级过滤，从源头不产生命名空间工具。连接器里的硬失败保留作兜底安全网。因此实跑第三方 provider 时**无需**再手动关多智能体特性。
