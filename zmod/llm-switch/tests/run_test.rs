...
async fn happy_path_chinese_content_not_corrupted() {
    let server = MockServer::start().await;
    // delta content contains Chinese; verify that the byte buffering path does not produce U+FFFD replacement characters
    ...
    // Chinese content must be passed through unchanged, must not be corrupted by replacement characters
    assert!(kinds.iter().any(|k| k.contains("OutputTextDelta")));
    assert!(kinds.iter().any(|k| k.contains("你好")), "Chinese content should be passed through unchanged, actual kinds: {kinds:?}");
}
